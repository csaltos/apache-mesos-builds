...
BUILD_IMAGE_NAME=${1:-debian-buster}
MESOS_VERSION=${2:-HEAD}
...
if [[ $MESOS_VERSION == HEAD ]]; then
    ...
    MESOS_LABEL_VERSION=$MESOS_VERSION
fi

OUTPUT=$BASE_DIR/output
OUTPUT_PACKAGE=$OUTPUT/mesos-$MESOS_LABEL_VERSION-$BUILD_IMAGE_NAME.deb

docker build -t mesos-build-$BUILD_IMAGE_NAME:$MESOS_LABEL_VERSION $TARGET_IMAGE

docker run mesos-build-$BUILD_IMAGE_NAME:$MESOS_LABEL_VERSION cat /builder/mesos-deb-packaging/pkg.deb > $OUTPUT_PACKAGE
